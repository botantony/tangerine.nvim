...
local function syn_match(group, pattern)
  _G.assert((nil ~= pattern), "Missing argument pattern on fnl/tangerine/output/logger.fnl:33")
  _G.assert((nil ~= group), "Missing argument group on fnl/tangerine/output/logger.fnl:33")
...
log.success = function(title, files, opts)
  _G.assert((nil ~= opts), "Missing argument opts on fnl/tangerine/output/logger.fnl:81")
  _G.assert((nil ~= files), "Missing argument files on fnl/tangerine/output/logger.fnl:81")
  _G.assert((nil ~= title), "Missing argument title on fnl/tangerine/output/logger.fnl:81")
...
log.failure = function(title, file, msg, opts)
  _G.assert((nil ~= opts), "Missing argument opts on fnl/tangerine/output/logger.fnl:93")
  _G.assert((nil ~= msg), "Missing argument msg on fnl/tangerine/output/logger.fnl:93")
  _G.assert((nil ~= file), "Missing argument file on fnl/tangerine/output/logger.fnl:93")
  _G.assert((nil ~= title), "Missing argument title on fnl/tangerine/output/logger.fnl:93")
  if env.conf(opts, {"compiler", "float"}) then
    local function _6_()
